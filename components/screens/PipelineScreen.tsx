...
import { pipelineData } from "@/lib/mock/generators";
...
export function PipelineScreen({ query }: { query: GlobalQuery }) {
  const env = useMemo(() => pipelineData(query), [query.from, query.to, query.compare]);
  const { data } = env;
      ...
      <Card title="Ideas" padded={false}>
        <DataTable
          ...
          columns={[
            ...
            { key: "submitted", label: "Submitted", render: (r: any) => formatDateShort(r.submitted) },
