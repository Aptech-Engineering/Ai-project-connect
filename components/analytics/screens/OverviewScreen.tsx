"use client";

import { Children } from "react";
import Link from "next/link";
import { useScreen } from "@/lib/analytics/api";
import { ScreenFallback } from "@/components/analytics/ScreenState";
import { KpiGrid } from "@/components/analytics/KpiTile";
import { ChartFrame } from "@/components/analytics/ChartFrame";
import { LineSeries } from "@/components/analytics/charts/LineSeries";
import { FunnelStrip } from "@/components/analytics/charts/FunnelStrip";
import { HorizontalBarList } from "@/components/analytics/charts/HorizontalBarList";
import { StatusPill } from "@/components/analytics/StatusPill";
import { Card } from "@/components/analytics/Card";
import { formatCurrency, formatNumber, formatPercent } from "@/lib/analytics/format";
import type { GlobalQuery, OverviewData } from "@/lib/analytics/types";

export function OverviewScreen({ query }: { query: GlobalQuery }) {
  const { envelope, loading, error, refresh } = useScreen<OverviewData>("overview", query);
  if (!envelope) return <ScreenFallback loading={loading} error={error} onRetry={refresh} />;
  const { data } = envelope;

  return (
    <div className="space-y-5">
      <KpiGrid kpis={data.kpis} />

      <div className="grid gap-4 lg:grid-cols-2">
        <ChartFrame
          title="Visitors per day"
          ariaLabel="Line chart of visitors per day for the selected range"
          chart={<LineSeries points={data.trend.visitors.points} color="#f26b22" valueFormatter={(v) => formatNumber(v, true)} />}
          tableColumns={[
            { key: "t", label: "Date" },
            { key: "value", label: "Visitors", align: "right" },
            { key: "previous", label: "Comparison", align: "right" },
          ]}
          tableRows={data.trend.visitors.points}
          tableFilename="overview-visitors"
        />
        <ChartFrame
          title="Ideas submitted per day"
          ariaLabel="Line chart of ideas submitted per day for the selected range"
          chart={<LineSeries points={data.trend.ideas.points} color="#14a38b" valueFormatter={(v) => formatNumber(v, true)} />}
          tableColumns={[
            { key: "t", label: "Date" },
            { key: "value", label: "Ideas submitted", align: "right" },
            { key: "previous", label: "Comparison", align: "right" },
          ]}
          tableRows={data.trend.ideas.points}
          tableFilename="overview-ideas"
        />
      </div>

      <Card title="Application funnel" subtitle="Visitors → idea form opened → fee paid → submitted → quoted → accepted">
        <FunnelStrip steps={data.funnel} />
      </Card>

      <div className="grid gap-4 lg:grid-cols-2">
        <Card title="Needs attention" subtitle="Overdue projects, transfers, refunds and stale projects">
          <div className="space-y-4">
            <AttentionGroup label="Overdue projects" status="critical" empty="Nothing overdue.">
              {data.attention.overdue.map((r) => (
                <AttentionRow key={r.code} link={r.link} primary={r.title} secondary={`${r.daysOverdue} day${r.daysOverdue === 1 ? "" : "s"} overdue`} />
              ))}
            </AttentionGroup>
            <AttentionGroup label="Transfers waiting" status="warning" empty="No transfers waiting to be confirmed.">
              {data.attention.transfers.map((r) => (
                <AttentionRow
                  key={r.reference}
                  link={r.link}
                  primary={r.reference}
                  secondary={`${formatCurrency(r.amount)} · waiting ${r.daysWaiting} day${r.daysWaiting === 1 ? "" : "s"}`}
                />
              ))}
            </AttentionGroup>
            <AttentionGroup label="Refunds due" status="serious" empty="No refunds queued.">
              {data.attention.refunds.map((r) => (
                <AttentionRow key={r.reference} link={r.link} primary={r.reference} secondary={`${formatCurrency(r.amount)} · refund ${r.refundStatus.toLowerCase()}`} />
              ))}
            </AttentionGroup>
            <AttentionGroup label="Stale projects" status="serious" empty="Every active project has a recent client update.">
              {data.attention.stale.map((r) => (
                <AttentionRow key={r.code} link={r.link} primary={r.title} secondary={`No client update in ${r.daysSinceClientUpdate} days`} />
              ))}
            </AttentionGroup>
          </div>
        </Card>

        <Card title="Top sources" subtitle="Top 5 traffic sources with application conversion">
          <HorizontalBarList rows={data.topSources} format="number" topN={5} />
        </Card>
      </div>
    </div>
  );
}

function AttentionGroup({
  label,
  status,
  empty,
  children,
}: {
  label: string;
  status: "good" | "warning" | "serious" | "critical";
  empty: string;
  children: React.ReactNode;
}) {
  const count = Children.count(children);
  return (
    <div>
      <div className="mb-1.5 flex items-center gap-2">
        <StatusPill status={count ? status : "good"} label={label} />
      </div>
      <div className="space-y-1.5">{count ? children : <p className="px-2 text-xs text-muted">{empty}</p>}</div>
    </div>
  );
}

function AttentionRow({ link, primary, secondary }: { link: string; primary: string; secondary: string }) {
  return (
    <Link href={link} className="focus-ring flex items-center justify-between gap-3 rounded-lg px-2 py-1.5 text-xs hover:bg-mist">
      <span className="truncate text-navy">{primary}</span>
      <span className="shrink-0 tabular-nums text-muted">{secondary}</span>
    </Link>
  );
}
